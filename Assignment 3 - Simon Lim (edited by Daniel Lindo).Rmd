...
Below you can find the code for a stacked bar chart
```{r}
...
Arg_vs_Can <- NAbirdBOLD %>%
  ...
  select(processid, order_name, species_name, country) %>%
  
  filter(country == c("Argentina", "Canada")) %>%
  
  filter(!is.na(species_name))

#Lets make this into a stacked bar chart now, this way we can visualise the species that belong to the order Anseriformes in Canada and Argentina. 

ggplot(data = Arg_vs_Can, aes(x = country, fill = species_name)) +
  
  geom_bar(colour = "black") +
  
  labs(x = "Country", y = "Number of Samples", title = "Stacked Bar Graph of Anseriformes between Argentina and Canada", fill = "Species Name") +
